...
public class VerticalMovement extends BasePrioritizable implements StrategyMarioAction {
    ...
    public void execute() {
        ...
        if (SingletonGame.instance().getKeyStatus(KeyEvent.VK_W) == KeyStatus.PRESSED) {
            if (mario.isFalling()) {
                if (speedY > 0) {
                ...
                SingletonSoundManager.instance().playSound("jumpSmall.wav");
            }
        } else {
            if (speedY > 0) {
