...
public class HorizontalMovement extends BasePrioritizable implements MarioAction {
    ...
    public HorizontalMovement(Mario m) {
    ...
    @Override
    public void execute() {
        float speedX = mario.getSpeedX();

        Direction currentDirection = Direction.horizontalDirectionFromSign((int) speedX);
        
        if (!mario.isFalling()) {
            handleGroundHorizontalMovement(mario, currentDirection);
        } else {
            handleAirHorizontalMovement(mario, currentDirection);
        }
    }
